...
> **注意**：bash tool 每次调用都是新 shell，`.zshrc`/`.bashrc` 中的 `export` 不会自动加载。
> 因此每次检查 env var 前都需要先 source 对应的 rc 文件。

1. 加载 shell 配置文件，然后读取环境变量 `$LLM_WIKI_VAULT`：

   ```bash
   source ~/.zshrc 2>/dev/null || source ~/.bashrc 2>/dev/null; echo "${LLM_WIKI_VAULT:-EMPTY}"
   ```

   如果输出 `EMPTY`，说明环境变量未设置。继续尝试直接从 rc 文件 grep：

   ```bash
   grep -i 'LLM_WIKI_VAULT' ~/.zshrc ~/.bashrc 2>/dev/null | head -1 || echo "NOT_FOUND"
   ```

   如果 grep 到了 `export LLM_WIKI_VAULT="/path/to/vault"`，提取该路径作为默认值。
...
2. 检查 `AGENTS.md` 中的 `LLM_WIKI_VAULT`（优先级更高）：
   ...
   如果输出了 `LLM_WIKI_VAULT="/path/to/vault"`，用此值**覆盖**第 1 步中读取的值。
