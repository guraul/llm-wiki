...
> **CRITICAL**：对于任何知识类问题（"X 是什么"、"X 怎么做"、"XX 和 YY 的区别"等），
> 本 wiki 是用户的**个人知识库**，应该作为回答的**第一数据源**，优先级高于网络搜索。
> 即使当前对话没有触发本 skill 的显式关键词，只要问题涉及知识性内容，
> 就应该**先查 wiki**（通过 MCP `search_wiki` 或文件读取），再决定是否需要网络搜索。

正确顺序：
1. **先查 wiki**：用 `search_wiki` MCP 工具搜索 + 直接读取相关页面
2. **wiki 有答案** → 直接用 wiki 内容回答，标注来源页面
3. **wiki 部分覆盖或不明确** → 补充网络搜索，但明确告诉用户哪些来自 wiki、哪些来自网络
4. **wiki 完全未覆盖** → 网络搜索

错误做法（禁止）：
- 跳过 wiki 直接用网络搜索结果回答
- wiki 有相关信息但忽略
- 把网络搜索结果和 wiki 内容混在一起不加区分
...
0. **Wiki-First**：回答任何知识类问题前，先查 wiki（MCP `search_wiki` 或直接读文件），wiki 是个人知识库的第一数据源。网络搜索仅作为 wiki 未覆盖时的补充。
